...
title: SQL Queries - Linq EF
description: Using SQL queries in Linq Entity Framework
author: smitpatel
ms.date: 09/19/2022
...
Linq Entity Framework allows you to drop down to SQL queries when working with a relational database. SQL queries are useful if the query you want can't be expressed using LINQ, or if a LINQ query causes EF to generate inefficient SQL. SQL queries can return regular entity types or [keyless entity types](xref:core/modeling/keyless-entity-types) that are part of your model.
...
> [!NOTE]
>
> <xref:Microsoft.EntityFrameworkCore.RelationalQueryableExtensions.FromSql%2A> was introduced in Linq EF 7.0. When using older versions, use <xref:Microsoft.EntityFrameworkCore.RelationalQueryableExtensions.FromSqlInterpolated%2A> instead.
...
You can compose on top of the initial SQL query using LINQ operators; Linq EF will treat your SQL as a subquery and compose over it in the database. The following example uses a SQL query that selects from a Table-Valued Function (TVF). And then composes on it using LINQ to do filtering and sorting.
...
Composing with LINQ requires your SQL query to be composable, since Linq EF will treat the supplied SQL as a subquery. Comopsable SQL queries generally begin with the `SELECT` keyword, and cannot contain SQL features that aren't valid in a subquery, such as:
...
> [!NOTE]
> This feature was introduced in Linq EF 7.0.
...
> [!NOTE]
>
> Prior to Linq EF 7.0, it was sometimes necessary to use the `ExecuteSql` APIs to perform a "bulk update" on the database, as above; this is considerably more efficient than querying for all matching rows and then using `SaveChanges` to modify them. Linq EF 7.0 introduced [ExecuteUpdate and ExecuteDelete](xref:core/what-is-new/ef-core-7#executeupdate-and-executedelete-bulk-updates), which made it possible to express efficient bulk update operations via LINQ. It's recommended to use those APIs whenever possible, instead of `ExecuteSql`.
