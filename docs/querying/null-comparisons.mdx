...
SQL databases operate on 3-valued logic (`true`, `false`, `null`) when performing comparisons, as opposed to the boolean logic of C#. When translating LINQ queries to SQL, Linq EF tries to compensate for the difference by introducing additional null checks for some elements of the query.
...
The third query introduces a null check. When `NullableInt` is `null` the comparison `Id <> NullableInt` yields `null`, which would be filtered out by `WHERE` operation. However, from the boolean logic perspective this case should be returned as part of the result. Hence Linq EF adds the necessary check to ensure that.
...
Many functions in SQL can only return a `null` result if some of their arguments are `null`. Linq EF takes advantage of this to produce more efficient queries.
...
In the second query, `null` results are filtered out from `String1` column explicitly. Linq EF can safely treat the `String1` column as non-nullable during comparison, resulting in a simpler query.
