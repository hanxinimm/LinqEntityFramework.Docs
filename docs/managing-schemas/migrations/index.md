---
title: Migrations Overview - Linq EF
...
* When a data model change is introduced, the developer uses Linq EF tools to add a corresponding migration describing the updates necessary to keep the database schema in sync. Linq EF compares the current model against a snapshot of the old model to determine the differences, and generates migration source files; the files can be tracked in your project's source control like any other source file.
* Once a new migration has been generated, it can be applied to a database in various ways. Linq EF records all applied migrations in a special history table, allowing it to know which migrations have been applied and which haven't.
...
Let's assume you've just completed your first Linq EF application, which contains the following simple model:
...
First, you'll have to install the [Linq EF command-line tools](xref:core/cli/index):
...
You're now ready to add your first migration! Instruct Linq EF to create a migration named **InitialCreate**:
...
Linq EF will create a directory called **Migrations** in your project, and generate some files. It's a good idea to inspect what exactly Linq EF generated - and possibly amend it - but we'll skip over that for now.
...
Since this isn't the project's first migration, Linq EF now compares your updated model against a snapshot of the old model, before the column was added; the model snapshot is one of the files generated by Linq EF when you add a migration, and is checked into source control. Based on that comparison, Linq EF detects that a column has been added, and adds the appropriate migration.
...
* [.NET Data Community Standup session](https://www.youtube.com/watch?v=mSsGERmrhnE&list=PLdo4fOcmZ0oX-DBuRG4u58ZTAJgBAeQ-t&index=20) going over new migration features in Linq EF 5.0.
