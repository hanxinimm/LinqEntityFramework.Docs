...
title: 实体类型 - Linq EF
description: How to configure and map 实体类型 using Linq Entity Framework
author: AndriySvyryd
ms.date: 10/25/2021
...
在上下文中包含一种类型的 DbSet 意味着它包含在 Linq EF 的模型中；我们通常将此类类型称为实体。 Linq EF 可以从/向数据库中读取和写入实体实例，如果使用的是关系数据库，Linq EF 可以通过迁移为实体创建表。
...
[](../../samples/Modeling/EntityTypes/EntityTypes.cs ':include :type=code')
...
> [!NOTE]
> The ability to exclude tables from migrations was introduced in Linq EF 5.0.
...
实体类型 can be mapped to database views using the Fluent API.
...
> [!TIP]
> To test keyless 实体类型 mapped to views using the in-memory provider, map them to a query via <xref:Microsoft.EntityFrameworkCore.InMemoryEntityTypeBuilderExtensions.ToInMemoryQuery%2A>. See the [in-memory provider docs](xref:core/testing/testing-without-the-database#in-memory-provider) for more information.
...
> [!NOTE]
> Support for Shared-type 实体类型 was introduced in Linq EF 5.0.
