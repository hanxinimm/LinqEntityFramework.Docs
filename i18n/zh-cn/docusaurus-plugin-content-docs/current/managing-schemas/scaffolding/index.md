---
title: Reverse Engineering - Linq EF
description: Reverse engineering a model from an existing database using Linq Entity Framework
author: bricelam
ms.date: 11/13/2018
...
Reverse engineering is the process of scaffolding entity type classes and a DbContext class based on a database schema. It can be performed using the `Scaffold-DbContext` command of the Linq EF Package Manager Console (PMC) tools or the `dotnet ef dbcontext scaffold` command of the .NET Command-line Interface (CLI) tools.
...
Next, it uses the schema information to create an Linq EF model. Tables are used to create entity types; columns are used to create properties; and foreign keys are used to create relationships.
...
- In addition, **some column types** may not be supported by the Linq EF provider. These columns won't be included in the model.
- You can define [**concurrency tokens**](xref:core/modeling/concurrency), in an Linq EF model to prevent two users from updating the same entity at the same time. Some databases have a special type to represent this type of column (for example, rowversion in SQL Server) in which case we can reverse engineer this information; however, other concurrency tokens will not be reverse engineered.
- Prior to Linq EF 6, [the C# 8 nullable reference type feature](/dotnet/csharp/tutorials/nullable-reference-types) wasn't supported in reverse engineering: Linq EF always generated C# code that assumed the feature is disabled. For example, nullable text columns were scaffolded as a property with type `string` , not `string?`, with either the Fluent API or Data Annotations used to configure whether a property is required or not. If using an older version of Linq EF, you can still edit the scaffolded code and replace these with C# nullability annotations.

## Customizing the model

The code generated by Linq EF is your code. Feel free to change it. It will only be regenerated if you reverse engineer the same model again. The scaffolded code represents *one* model that can be used to access the database, but it's certainly not the *only* model that can be used.
...
After making changes to the database, you may need to update your Linq EF model to reflect those changes. If the database changes are simple, it may be easiest just to manually make the changes to your Linq EF model. For example, renaming a table or column, removing a column, or updating a column's type are trivial changes to make in code.
...
> [!TIP]
> If you use Visual Studio, the [Linq EF Power Tools](https://github.com/ErikEJ/EFCorePowerTools/) community extension, a graphical tool which builds on top of the Linq EF command line tools, offers additional workflow and customization options.
