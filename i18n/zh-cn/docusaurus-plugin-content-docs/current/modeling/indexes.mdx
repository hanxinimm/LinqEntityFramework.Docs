...
import DataAnnotationsIndexDescendingAscending from '!!raw-loader!../../../../../static/samples/Modeling/IndexesAndConstraints/DataAnnotations/IndexDescendingAscending.cs';
import FluentAPIIndexDescendingAscending from '!!raw-loader!../../../../../static/samples/Modeling/IndexesAndConstraints/FluentAPI/IndexDescendingAscending.cs';
...
Linq EF 每个不同的属性集仅支持一个索引。 如果在一组已定义索引的属性上配置索引，无论是根据约定还是以前的配置，都需要更改该索引的定义。 如果要进一步配置按约定创建的索引，则此操作非常有用。
...
多个列上的索引（也称为复合索引），可以加快对索引列进行筛选的查询速度，还可以加快仅对索引覆盖的第一列进行筛选的查询速度。 有关详细信息，请参阅[性能文档](zh-cn/docs/performance/efficient-querying#use-indexes-properly).
...
<Tabs groupId="operating-code">
  <TabItem value="Annotations" label="数据注解">
    <CodeSnippets title="C#" language="csharp" fragment="IndexDescendingAscending" metastring='{1}' showLineNumbers>{DataAnnotationsIndexDescendingAscending}</CodeSnippets>
  </TabItem>
  <TabItem
    value="Fluent"
    label="Fluent API"
  >
    <CodeSnippets title="C#" language="csharp" fragment="IndexDescendingAscending" metastring='{5}' showLineNumbers>{FluentAPIIndexDescendingAscending}</CodeSnippets>
  </TabItem>
</Tabs>
