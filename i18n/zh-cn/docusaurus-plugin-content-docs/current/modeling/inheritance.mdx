...
import InheritanceDbSets from '!!raw-loader!../../../../../static/samples/Modeling/Inheritance/InheritanceDbSets.cs';
...
LEF 可以将 .NET 类型层次结构映射到数据库。 这允许你像往常一样使用基类型和派生类型在代码中编写 .NET 实体，并让 LEF 无缝创建适当的数据库架构、发出查询等。有关如何映射类型层次结构的实际细节取决于提供程序；本页介绍关系数据库上下文中的继承支持。

## 实体类型层次结构映射

按照约定，LEF 不会自动扫描基类型或派生类型；这意味着，如果要映射层次结构中的 CLR 类型，就必须在模型上显式指定该类型。 例如，仅指定层次结构的基类型不会导致 LEF Core 隐式包含其所有子类型。

以下示例将为 `Blog` 及其子类 `RssBlog` 公开 `DbSet`。 如果 `Blog` 有任何其他子类，它不会包含在模型中。

<CodeSnippets title="C#" language="csharp" fragment="InheritanceDbSets" metastring='{3-4}' showLineNumbers>{InheritanceDbSets}</CodeSnippets>
...
默认情况下，LEF 使用每个层次结构一张表 (TPH) 模式来映射继承。 TPH 使用单个表来存储层次结构中所有类型的数据，并使用鉴别器列来标识每行表示的类型。
...
在上面的示例中，EF 在层次结构的基本实体上隐式添加了鉴别器作为[影子属性](zh-cn/docs/modeling/shadow-properties)。 可以像配置任何其他属性一样配置此属性：
...
> [!NOTE]
 开发中... The table-per-type (TPT) feature was introduced in EF Core 5.0.
> 
...
> [!TIP]
无需在每个实体类型上调用 `ToTable`，当在根实体调用 `modelBuilder.Entity<Blog>().UseTptMappingStrategy()` 方法时，表名称将由 LEF 生成。
> 
