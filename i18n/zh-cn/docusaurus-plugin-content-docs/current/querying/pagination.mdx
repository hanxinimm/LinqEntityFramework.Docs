...
使用数据库实现分页的一种常见方法是在 SQL 中使用 `Paging` (OFFSETLIMIT) 。 给定页面大小为 10 个结果，可以使用 Linq EF 提取第三页，如下所示：
...
> [!NOTE]
大多数 SQL 数据库都支持更简单、更高效的上述版本，使用行值： WHERE (Date, Id) > (@lastDate, @lastId) Linq EF 目前不支持在 LINQ 查询中表示这一点，这是由 [#26822](https://github.com/dotnet/efcore/issues/26822) 跟踪的。
> 
...
* 有关 Linq EF 上简化密钥集分页的扩展，请参阅[MR.EntityFrameworkCore.KeysetPagination](https://github.com/mrahhal/MR.EntityFrameworkCore.KeysetPagination) 和 [MR.AspNetCore.Pagination](https://github.com/mrahhal/MR.AspNetCore.Pagination).
