...
title: 拆分查询
description: Translating query into single and split queries in SQL with Linq Entity Framework
author: smitpatel
ms.date: 10/03/2019
...
> [!NOTE]
> This feature was introduced in Linq EF 5.0, where it only worked when using `Include`. Linq EF 6.0 added support for split queries when loading related data in projections, without `Include`.
...
Linq EF uses single query mode by default in the absence of any configuration. Since it may cause performance issues, Linq EF generates a warning whenever following conditions are met:

- Linq EF detects that the query loads multiple collections.
