...
import IntroModel from '!!raw-loader!../../../../static/samples/Intro/Model.cs';

import IntroProgram from '!!raw-loader!../../../../static/samples/Intro/Program.cs';
...
* 从现有数据库生成模型。
* 手动编写模型以匹配数据库。
* 创建模型后，使用 [LINQ EF 迁移](/docs/managing-schemas/migrations/) 从模型创建数据库。迁移允许随着模型的变化而发展数据库。

<CodeBlock title="C#" language="csharp" showLineNumbers>{IntroModel}</CodeBlock>

## 查询

使用实体类的查询语法[Lambda表达式](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/operators/lambda-expressions) 从数据库检索实体类的实例。 有关详细信息，请参阅查询数据。

<CodeSnippets title="C#" language="csharp" fragment="Querying" showLineNumbers>{IntroProgram}</CodeSnippets>

## 保存数据

使用实体类的的修改语法[Lambda表达式](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/operators/lambda-expressions) 从数据库中创建、删除和修改数据。请参阅 [保存数据]（外部参照：核心/保存/索引）以了解更多信息。

<CodeSnippets title="C#" language="csharp" fragment="SavingData" showLineNumbers>{IntroProgram}</CodeSnippets>
